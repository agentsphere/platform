...
pub async fn rotation_loop(state: ObserveState, cancel: tokio_util::sync::CancellationToken) {
        ...
        tokio::select! {
            ...
            () = tokio::time::sleep(std::time::Duration::from_secs(900)) => {
                ...
                async {
                    ...
                    if !had_error {
                        tracing::info!("parquet rotation completed successfully");
                    }
...
/// Rotate log entries older than the configured retention to `MinIO` Parquet.
#[tracing::instrument(skip(state), err)]
pub async fn rotate_logs(state: &ObserveState) -> Result<u64, ObserveError> {
    let hours = i64::from(state.config.parquet_log_retention_hours);
...
/// Rotate spans older than the configured retention to `MinIO` Parquet.
#[tracing::instrument(skip(state), err)]
pub async fn rotate_spans(state: &ObserveState) -> Result<u64, ObserveError> {
    let hours = i64::from(state.config.parquet_log_retention_hours);
    let cutoff = chrono::Utc::now() - chrono::Duration::hours(hours);
...
/// Rotate metric samples older than the configured retention to `MinIO` Parquet.
#[tracing::instrument(skip(state), err)]
pub async fn rotate_metrics(state: &ObserveState) -> Result<u64, ObserveError> {
    let hours = i64::from(state.config.parquet_metric_retention_hours);
    let cutoff = chrono::Utc::now() - chrono::Duration::hours(hours);
...
mod tests {
    ...
    fn build_span_batch_all_optionals_none() {
        ...
        let batch = build_span_batch(&[row]).unwrap();
        assert_eq!(batch.num_rows(), 1);
        assert_eq!(batch.column(2).null_count(), 1); // parent_span_id
        assert_eq!(batch.column(7).null_count(), 1); // duration_ms
        assert_eq!(batch.column(9).null_count(), 1); // attributes
    }
