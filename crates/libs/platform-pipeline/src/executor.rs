...
use crate::state::{PipelineServices, PipelineState};
...
pub async fn run<Svc: PipelineServices>(
    ...
    state.task_heartbeat.register("pipeline_executor", 10);
            ...
            _ = interval.tick() => {
                ...
                async {
                    match poll_pending(&state).await {
                        Ok(()) => state.task_heartbeat.heartbeat("pipeline_executor"),
                        Err(e) => {
                            state.task_heartbeat.report_error("pipeline_executor", &e.to_string());
...
async fn poll_pending<Svc: PipelineServices>(
    state: &PipelineState<Svc>,
...
async fn execute_pipeline<Svc: PipelineServices>(
    state: &PipelineState<Svc>,
    pipeline_id: Uuid,
) -> Result<(), PipelineError> {
    // Pipeline already claimed by poll_pending() — just fetch the project_id.
    let project_id =
        sqlx::query_scalar::<_, Uuid>("SELECT project_id FROM pipelines WHERE id = $1")
            .bind(pipeline_id)
    ...
    let pipeline = PipelineQueryRow {
        git_ref: pipeline_row.get("git_ref"),
        commit_sha: pipeline_row.get("commit_sha"),
        owner_id: pipeline_row.get("owner_id"),
        triggered_by: pipeline_row.get("triggered_by"),
        version: pipeline_row.get("version"),
        trigger: pipeline_row.get("trigger"),
        project_name: pipeline_row.get("project_name"),
        namespace_slug: pipeline_row.get("namespace_slug"),
        owner_name: pipeline_row.get("owner_name"),
    };
    ...
    let pipeline_namespace = platform_k8s::pipeline_namespace_name(
...
async fn finalize_pipeline<Svc: PipelineServices>(
    state: &PipelineState<Svc>,
    pipeline_id: Uuid,
    project_id: Uuid,
    all_succeeded: bool,
    ...
    sqlx::query("UPDATE pipelines SET status = $2, finished_at = now() WHERE id = $1")
        .bind(pipeline_id)
...
async fn ensure_pipeline_namespace<Svc: PipelineServices>(
    state: &PipelineState<Svc>,
    namespace: &str,
    project_id: Uuid,
) -> Result<(), PipelineError> {
    ...
    if !state.config.dev_mode {
        let _ = platform_k8s::ensure_network_policy(
            &state.kube,
            namespace,
            &state.config.platform_namespace,
        )
...
async fn run_all_steps<Svc: PipelineServices>(
    state: &PipelineState<Svc>,
    pipeline_id: Uuid,
    project_id: Uuid,
...
async fn run_steps_sequential<Svc: PipelineServices>(
    state: &PipelineState<Svc>,
    pipeline_id: Uuid,
    project_id: Uuid,
    pipeline: &PipelineMeta,
    ...
    for step in steps {
        ...
        if !crate::definition::step_matches(condition.as_ref(), &pipeline.trigger_type, branch) {
            tracing::info!(
                step = %step.name,
...
async fn run_steps_dag<Svc: PipelineServices>(
    state: &PipelineState<Svc>,
    pipeline_id: Uuid,
    project_id: Uuid,
    pipeline: &PipelineMeta,
...
#[allow(clippy::too_many_arguments)]
async fn execute_step_dispatch<Svc: PipelineServices>(
    state: &PipelineState<Svc>,
    pods: &Api<Pod>,
    pipeline_id: Uuid,
    project_id: Uuid,
...
fn step_condition_from_row(step: &StepRow) -> Option<crate::definition::StepCondition> {
    if step.condition_events.is_empty() && step.condition_branches.is_empty() {
        return None;
    }
    Some(crate::definition::StepCondition {
...
#[allow(clippy::too_many_arguments, clippy::too_many_lines)]
async fn execute_single_step<Svc: PipelineServices>(
    state: &PipelineState<Svc>,
    pods: &Api<Pod>,
    pipeline_id: Uuid,
    project_id: Uuid,
    ...
    let resolved_image = crate::definition::expand_step_env(&step.image, &env_pairs);
    ...
    sqlx::query("UPDATE pipeline_steps SET status = 'running' WHERE id = $1")
        .bind(step.id)
        .execute(&state.pool)
        .await?;
    ...
    match result {
        Ok(exit_code) => {
            ...
            sqlx::query(
                "UPDATE pipeline_steps SET status = $2, exit_code = $3, duration_ms = $4, log_ref = $5, finished_at = now() WHERE id = $1",
            )
            .bind(step.id)
            .bind(status)
            .bind(exit_code)
...
#[allow(clippy::too_many_arguments)]
async fn run_step<Svc: PipelineServices>(
    pods: &Api<Pod>,
    pod_name: &str,
    pod_spec: &Pod,
    state: &PipelineState<Svc>,
    ...
    step_id: Uuid,
    step_name: &str,
    artifact_defs: &[crate::definition::ArtifactDef],
...
fn extract_artifact_defs(
    step_config: Option<&serde_json::Value>,
) -> Vec<crate::definition::ArtifactDef> {
    step_config
        .and_then(|c| c.get("artifacts"))
        .and_then(|v| serde_json::from_value::<Vec<crate::definition::ArtifactDef>>(v.clone()).ok())
        .unwrap_or_default()
}
...
async fn collect_step_artifacts<Svc: PipelineServices>(
    pods: &Api<Pod>,
    pod_name: &str,
    state: &PipelineState<Svc>,
    pipeline_id: Uuid,
...
async fn unpack_and_store<Svc: PipelineServices>(
    tar_bytes: &[u8],
    state: &PipelineState<Svc>,
    pipeline_id: Uuid,
    step_id: Uuid,
...
async fn capture_logs<Svc: PipelineServices>(
    pods: &Api<Pod>,
    pod_name: &str,
    state: &PipelineState<Svc>,
    pipeline_id: Uuid,
...
async fn create_git_auth_token<Svc: PipelineServices>(
    state: &PipelineState<Svc>,
    pipeline_id: Uuid,
    project_id: Uuid,
    triggered_by: Option<Uuid>,
    ...
    let user_id = if let Some(uid) = triggered_by {
        ...
        sqlx::query_scalar::<_, Uuid>("SELECT owner_id FROM projects WHERE id = $1")
            .bind(project_id)
            .fetch_one(&state.pool)
            .await?
    };
    ...
    sqlx::query(
        "INSERT INTO api_tokens (id, user_id, name, token_hash, project_id, expires_at)
           VALUES ($1, $2, $3, $4, $5, now() + interval '1 hour')",
    )
    .bind(Uuid::new_v4())
    .bind(user_id)
    .bind(format!("pipeline-git-{pipeline_id}"))
    .bind(&token_hash)
    .bind(project_id)
    .execute(&state.pool)
    .await?;
...
async fn cleanup_git_auth_token<Svc: PipelineServices>(
    state: &PipelineState<Svc>,
    token_hash: &str,
) {
...
async fn create_registry_secret<Svc: PipelineServices>(
    state: &PipelineState<Svc>,
    pipeline_id: Uuid,
    project_id: Uuid,
    project_name: &str,
...
async fn cleanup_registry_secret<Svc: PipelineServices>(
    state: &PipelineState<Svc>,
    pipeline_id: Uuid,
    token_hash: &str,
    namespace: &str,
...
fn node_registry_url(config: &crate::config::PipelineConfig) -> Option<&str> {
    config
        .node_registry_url
        .as_deref()
...
async fn resolve_pipeline_secrets<Svc: PipelineServices>(
    state: &PipelineState<Svc>,
    project_id: Uuid,
) -> Vec<(String, String)> {
    ...
    let Ok(master_key) = platform_secrets::engine::parse_master_key(master_key_hex) else {
        return Vec::new();
    };
    match platform_secrets::engine::query_scoped_secrets(
        &state.pool,
        &master_key,
        project_id,
        &["pipeline", "agent", "all"],
...
async fn create_pipeline_otlp_token<Svc: PipelineServices>(
    state: &PipelineState<Svc>,
    project_id: Uuid,
    pipeline_id: Uuid,
) -> Option<String> {
    let owner_id: Uuid = match sqlx::query_scalar::<_, Uuid>(
...
#[allow(clippy::too_many_arguments)]
fn build_env_vars_full<Svc: PipelineServices>(
    state: &PipelineState<Svc>,
    pipeline_id: Uuid,
    project_id: Uuid,
    meta: &PipelineMeta,
        ...
        && let Some(map) = env_json.as_object()
    {
        ...
        for (key, val) in map {
            if let Some(v) = val.as_str() {
                let expanded = crate::definition::expand_step_env(v, &existing_pairs);
...
#[allow(clippy::too_many_lines)]
async fn execute_deploy_test_step<Svc: PipelineServices>(
    state: &PipelineState<Svc>,
    pipeline_id: Uuid,
    project_id: Uuid,
    pipeline: &PipelineMeta,
    ...
    let dt: crate::definition::DeployTestDef =
    ...
    let test_image = crate::definition::expand_step_env(&dt.test_image, &env_pairs);
    ...
    let short_id = &pipeline_id.to_string()[..8];
    let ns_name = platform_k8s::test_namespace_name(
        state.config.ns_prefix.as_deref(),
        &pipeline.namespace_slug,
        short_id,
    );
    ...
    let manifest_content_opt = if manifests_path.ends_with('/') {
        crate::trigger::read_dir_at_ref(std::path::Path::new(&repo_path), branch, manifests_path)
            .await
    } else {
        crate::trigger::read_file_at_ref(std::path::Path::new(&repo_path), branch, manifests_path)
            .await
    };
    ...
    // Render manifests and apply to test namespace
    let render_vars = serde_json::json!({
    ...
    // 4. Render + apply manifests to test namespace
    if let Err(e) = state
        .services
        .render_and_apply(&state.kube, &manifest_content, &render_vars, &ns_name, None)
        .await
    {
...
async fn capture_deployment_logs<Svc: PipelineServices>(
    state: &PipelineState<Svc>,
    namespace: &str,
    pipeline_id: Uuid,
    step_name: &str,
...
async fn execute_gitops_sync_step<Svc: PipelineServices>(
    state: &PipelineState<Svc>,
    pipeline_id: Uuid,
    project_id: Uuid,
    pipeline: &PipelineMeta,
...
#[allow(clippy::too_many_lines)]
async fn execute_gitops_sync_inner<Svc: PipelineServices>(
    state: &PipelineState<Svc>,
    _pipeline_id: Uuid,
    project_id: Uuid,
    pipeline: &PipelineMeta,
    ...
    let platform_yaml = state
        .services
        .ops_read_file(std::path::Path::new(repo_path), sha, ".platform.yaml")
        .await;

    let platform_file: Option<crate::definition::PlatformFile> = platform_yaml
        .as_ref()
    ...
    if target_branch != ops_repo_branch
        && let Err(e) = state
            .services
            .ops_sync_from_project(project_id, std::path::Path::new(repo_path), target_branch)
            .await
    {
    ...
    {
        let deploy_content = state
            .services
            .ops_read_dir_yaml(&ops_path, target_branch, "deploy/")
            .await
            .unwrap_or_default();
        if let Err(e) =
            crate::definition::validate_canary_service_refs(&deploy.specs, &deploy_content)
        {
    ...
    let version_info =
        crate::trigger::read_version_at_ref(std::path::Path::new(repo_path), sha).await;
    ...
    if let Some(var_path) = var_path
        && let Some(var_content) = state
            .services
            .ops_read_file(std::path::Path::new(repo_path), sha, var_path)
            .await
    {
    ...
    // Build key=value pairs from the values JSON object
    let values_obj = values.as_object().cloned().unwrap_or_default();
    let kv_strings: Vec<(String, String)> = values_obj
        .iter()
        .map(|(k, v)| {
            let val = match v {
        ...
        .collect();
    let kv_refs: Vec<(&str, &str)> = kv_strings
        .iter()
        .map(|(k, v)| (k.as_str(), v.as_str()))
        .collect();

    let ops_commit_sha = match state
        .services
        .ops_commit_values(
            &ops_path,
            target_branch,
            &kv_refs,
            &format!("chore: update {environment} values"),
        )
        .await
    {
        Ok(sha) => sha,
    ...
    let event = platform_types::PlatformEvent::OpsRepoUpdated {
        project_id,
        ops_repo_id,
    ...
    if let Err(e) = platform_types::events::publish(&state.valkey, &event).await {
        ...
        && !pf.flags.is_empty()
    {
        let flag_defs: Vec<(String, serde_json::Value, Option<String>)> = pf
            ...
            .collect();
        let flag_event = platform_types::PlatformEvent::FlagsRegistered {
            project_id,
            flags: flag_defs,
        };
        let _ = platform_types::events::publish(&state.valkey, &flag_event).await;
    }
...
#[tracing::instrument(skip(state, step), fields(%pipeline_id, %project_id), err)]
async fn execute_deploy_watch_step<Svc: PipelineServices>(
    state: &PipelineState<Svc>,
    pipeline_id: Uuid,
    project_id: Uuid,
    step: &StepRow,
...
async fn inject_test_namespace_secrets<Svc: PipelineServices>(
    state: &PipelineState<Svc>,
    project_id: Uuid,
    project_name: &str,
    namespace: &str,
    ...
    if let Some(ref master_key_str) = state.config.master_key
        && let Ok(mk) = platform_secrets::engine::parse_master_key(master_key_str)
    {
        let rows = sqlx::query(
        ...
        for row in &rows {
            ...
            match platform_secrets::engine::decrypt(&encrypted, &mk, None) {
                Ok(val) => {
    ...
    if let Err(e) = state
        .services
        .ensure_scoped_tokens(project_id, "test")
        .await
    {
        tracing::warn!(error = %e, %namespace, "failed to ensure scoped tokens for test namespace");
    }
...
async fn fire_build_webhook<Svc: PipelineServices>(
    state: &PipelineState<Svc>,
    project_id: Uuid,
    pipeline_id: Uuid,
    status: &str,
    ...
    state
        .services
        .fire_webhooks(project_id, "build", &payload)
        .await;
}
...
#[tracing::instrument(skip(state), fields(%pipeline_id), err)]
pub async fn cancel_pipeline<Svc: PipelineServices>(
    state: &PipelineState<Svc>,
    pipeline_id: Uuid,
) -> Result<(), PipelineError> {
    // Fetch current status and validate transition via state machine
...
mod tests {
    ...
    fn test_pipeline_config() -> crate::config::PipelineConfig {
        crate::config::PipelineConfig {
            ...
            ns_prefix: None,
            gateway_namespace: "istio-system".into(),
            registry_url: None,
    ...
    fn node_registry_url_prefers_node_url() {
        let config = crate::config::PipelineConfig {
            ...
            node_registry_url: Some("node.example.com:5000".into()),
    ...
    fn node_registry_url_both_set_prefers_node() {
        let config = crate::config::PipelineConfig {
            ...
            node_registry_url: Some("node-registry.com:5000".into()),
            ..test_pipeline_config()
        };
